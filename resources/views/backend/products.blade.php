...
<div class="space12">
    <div class="btn-group btn-group-lg">
        ...
        <a class="btn btn-default hidden-xs" href="{!! langRoute('admin.product.create') !!}">
...
<div class="products">
<a class="fa fa-plus add-btn" href="{{ url(getLang().'/admin/product/create') }}">Add Product</a>
<table class="table">
	<tr id="table-header">
	...
	<tr>
		<td><a href="{{ url(getLang().'/product/'.$product->id.'-'.Str::slug($product->name).'/show') }}">{{ $product->id }}</a></td>
		<td>{{ $product->name }}</td>
		...
		<td><a href="{{ url(getLang().'/admin/product/'.$product->id.'/edit') }}" class="fa fa-pencil-square-o"></a></td>
    	<td><a href="{{ url(getLang().'/product/'.$product->id.'/delete') }}" class="fa fa-times"></a></td>
    </tr>
	@endforeach
</table>
{!! $products->appends([
